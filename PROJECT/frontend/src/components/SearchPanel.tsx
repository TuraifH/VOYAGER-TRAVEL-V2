...
export function SearchResults() {
  const { searchResults, setPlaces, setPinned, setFlyTo, setSelected, setShowDiscovery, setDest, setMode } = useApp();
  ...
  if (searchResults.length === 0) {
    return (
      <div className="results-empty muted small">
        Results will appear here. Search for a place or run a Nearby search.
      </div>
    );
  }
...
export default function SearchInput() {
  const { userLoc, setPlaces, setSearchResults, setNearbyBase, nearbyBase, pinned, setPinned, clearTransient } = useApp();
  ...
  const switchTab = (t: "specific" | "nearby") => {
    clearTransient();
    setTab(t);
  };
  ...
  const onRadiusChange = (v: number) => {
    setRadiusKm(v);
    if (nearbyBase) setNearbyBase({ ...nearbyBase, radiusM: v * 1000 });
  };

  return (
    <div className="search-panel">
      <div className="tabs row">
        <button className={`chip ${tab === "specific" ? "active" : ""}`} onClick={() => switchTab("specific")}>Search</button>
        <button className={`chip ${tab === "nearby" ? "active" : ""}`} onClick={() => switchTab("nearby")}>Nearby</button>
      </div>
      ...
      {tab === "specific" ? (
        ...
        <div className="nearby">
          ...
          <div className="radius row">
            ...
            <input type="range" min={0.5} max={10} step={0.5} value={radiusKm} onChange={(e) => onRadiusChange(Number(e.target.value))} />
